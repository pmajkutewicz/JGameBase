...
import jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.LZ.OutWindow;
import jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.RangeCoder.BitTreeDecoder;
...
/*
 ...
 #endif
 */
// OLD CODE public class Decoder implements jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressCoder , jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressSetDecoderProperties2
public class Decoder extends java.io.InputStream
    // _ST_MODE
    implements jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressCoder,
    jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressSetDecoderProperties2,
    jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressGetInStreamProcessedSize,
    // #ifdef _ST_MODE
    jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressSetInStream,
    jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressSetOutStreamSize
// #endif

{
  class LenDecoder {
    ...
    public void Init() {
      jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.RangeCoder.Decoder
    ...
    public int Decode(
        final jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.RangeCoder.Decoder rangeDecoder,
        final int posState) throws IOException {
  ...
  jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.RangeCoder.Decoder m_RangeDecoder = new jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.RangeCoder.Decoder();
  ...
  public int ReleaseInStream() throws IOException {
    ...
    return jgamebase.plugins.extractor.sevenzip.SevenZip.HRESULT.S_OK;
  }

  @Override
  public int SetInStream(final java.io.InputStream inStream) { // jgamebase.plugins.extractor.sevenzip.Common.ISequentialInStream
  ...
  @Override
  public int SetOutStreamSize(final long outSize /* const UInt64 *outSize */) {
    _outSizeDefined = (outSize != jgamebase.plugins.extractor.sevenzip.SevenZip.ICompressSetOutStreamSize.INVALID_OUTSIZE);
    if (_outSizeDefined) {
      _outSize = outSize;
    }
  ...
  @Override
  public int read(final byte[] data, final int off, final int size) throws IOException {
    if (off != 0) {
      throw new java.io.IOException(
          "LZMA Decoder - read(byte [] data, int off != 0, int size)) not implemented");
    }

    final long startPos = m_OutWindow.GetProcessedSize();
  ...
  public int CodeReal(final java.io.InputStream inStream, // ,
                                                          // ISequentialInStream
      final java.io.OutputStream outStream, // ISequentialOutStream
      final long outSize, final ICompressProgressInfo progress // useless_progress
  ) throws IOException {
    ...
    for (;;) {
      ...
      if (progress != null) {
        final long inSize = m_RangeDecoder.GetProcessedSize();
        final long nowPos64 = m_OutWindow.GetProcessedSize();
  ...
  {
    if (_outSizeDefined) {
      final long rem = _outSize - m_OutWindow.GetProcessedSize();
      if (curSize > rem) {
        curSize = (int) rem;
      }
    }
    ...
    while (curSize > 0) {
      ...
      if (m_RangeDecoder.DecodeBit(m_IsMatchDecoders, (state << Base.kNumPosStatesBitsMax)
          + posState) == 0) {
        final LiteralDecoder.Decoder2 decoder2 = m_LiteralDecoder.GetDecoder((int) nowPos64,
            prevByte);
        ...
        if (m_RangeDecoder.DecodeBit(m_IsRepDecoders, state) == 1) {
          ...
          if (posSlot >= Base.kStartPosModelIndex) {
            ...
            if (posSlot < Base.kEndPosModelIndex) {
              rep0 += BitTreeDecoder.ReverseDecode(m_PosDecoders, rep0 - posSlot - 1,
  ...
  @Override
  public boolean SetDecoderProperties2(final byte[] properties) {
