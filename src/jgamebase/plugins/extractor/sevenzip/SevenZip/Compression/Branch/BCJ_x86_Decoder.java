...
public class BCJ_x86_Decoder implements ICompressFilter {
  ...
  static final boolean[] kMaskToAllowedStatus = { true, true, true, false, true, false, false,
      false };

  static final int[] kMaskToBitNumber = { 0, 1, 2, 2, 3, 3, 3, 3 };

  static final boolean Test86MSByte(final int b) {
    return (((b) == 0) || ((b) == 0xFF));
  }

  static final int x86_Convert(final byte[] buffer, final int endPos, final int nowPos,
      final int[] prevMask, final int[] prevPos, final boolean encoding) {
    int bufferPos = 0;
    int limit;
    ...
    while (bufferPos <= limit) {
      ...
      if (Test86MSByte(b) && kMaskToAllowedStatus[(prevMask[0] >> 1) & 0x7]
          && ((prevMask[0] >>> 1) < 0x10)) {
        int src = ((b) << 24) | ((buffer[bufferPos + 3] & 0xFF) << 16)
            | ((buffer[bufferPos + 2] & 0xFF) << 8) | (buffer[bufferPos + 1] & 0xFF);
  ...
  public int SubFilter(final byte[] data, final int size) {
  ...
  @Override
  public int Filter(final byte[] data, final int size) {
    final int processedSize = SubFilter(data, size);
