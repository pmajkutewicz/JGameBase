...
public class J7zip {
  ...
  static void listing(final IInArchive archive, final Vector<String> listOfNames,
      final boolean techMode) {
    ...
    for (int i = 0; i < archive.size(); i++) {
      final SevenZipEntry item = archive.getEntry(i);

      final DateFormat formatter = DateFormat.getDateTimeInstance(DateFormat.SHORT,
          DateFormat.SHORT);
      final String str_tm = formatter.format(new java.util.Date(item.getTime()));
    ...
    if (!techMode) {
      System.out.println("-------------- ----- ------------ ------------  ------------");
      System.out.print(String.format("                    %13d%13d %d files", size, packSize,
          nbFiles));
    }
  }

  static void testOrExtract(final IInArchive archive, final Vector<String> listOfNames,
  ...
  public static void main(final String[] args) throws Exception {
    System.out.println("\nJ7zip 4.43 ALPHA 2 (" + Runtime.getRuntime().availableProcessors()
        + " CPUs)");
    ...
    final MyRandomAccessFile istream = new MyRandomAccessFile(filename, "r");

    final IInArchive archive = new Handler();

    final int ret = archive.Open(istream);
