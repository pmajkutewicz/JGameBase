...
public class LzmaBench {
  ...
  static class CBenchRandomGenerator {
    ...
    public void Set(final int bufferSize) {
    ...
    int GetLogRandBits(final int numBits) {
  ...
  static class MyOutputStream extends java.io.OutputStream {
    ...
    public MyOutputStream(final byte[] buffer) {
      _buffer = buffer;
  ...
  static class MyInputStream extends java.io.InputStream {
    ...
    public MyInputStream(final byte[] buffer, final int size) {
      _buffer = buffer;
      _size = size;
    }
  ...
  static class CProgressInfo implements ICodeProgress {
    ...
    @Override
    public void SetProgress(final long inSize, final long outSize) {
  ...
  static int GetLogSize(final int size) {
  ...
  static long MyMultDiv64(final long value, final long elapsedTime) {
    long freq = 1000; // ms
  ...
  static long GetCompressRating(final int dictionarySize, final long elapsedTime, final long size) {
  ...
  static long GetDecompressRating(final long elapsedTime, final long outSize, final long inSize) {
    final long numCommands = (inSize * 220) + (outSize * 20);
    return MyMultDiv64(numCommands, elapsedTime);
  }

  static long GetTotalRating(final int dictionarySize, final long elapsedTimeEn, final long sizeEn,
      final long elapsedTimeDe, final long inSizeDe, final long outSizeDe) {
    return (GetCompressRating(dictionarySize, elapsedTimeEn, sizeEn) + GetDecompressRating(
        elapsedTimeDe, inSizeDe, outSizeDe)) / 2;
  }

  static void PrintValue(final long v) {
    String s = "";
    s += v;
  ...
  static void PrintRating(final long rating) {
    PrintValue(rating / 1000000);
    System.out.print(" MIPS");
  }

  static void PrintResults(final int dictionarySize, final long elapsedTime, final long size,
      final boolean decompressMode, final long secondSize) {
  ...
  static public int LzmaBenchmark(final int numIterations, final int dictionarySize)
      throws Exception {
    if (numIterations <= 0) {
      return 0;
    }
    ...
    final jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.LZMA.Encoder encoder = new jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.LZMA.Encoder();
    final jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.LZMA.Decoder decoder = new jgamebase.plugins.extractor.sevenzip.SevenZip.Compression.LZMA.Decoder();
    ...
    rg.Set(kBufferSize);
    rg.Generate();
    final jgamebase.plugins.extractor.sevenzip.Common.CRC crc = new jgamebase.plugins.extractor.sevenzip.Common.CRC();
    crc.Init();
    ...
    System.out.print("     ");
    PrintResults(dictionarySize, totalDecodeTime, kBufferSize * (long) numIterations, true,
        totalCompressedSize);
